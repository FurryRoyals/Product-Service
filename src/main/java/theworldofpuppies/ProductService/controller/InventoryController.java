...
import theworldofpuppies.ProductService.service.inventory.InventoryService;
...
public class InventoryController {
            ...
            if (availabilityRequests != null && !availabilityRequests.isEmpty()) {
                ...
                if (isAvailable) {
                    ...
                    return ResponseEntity.status(BAD_REQUEST)
                            .body(new InventoryResponse(response.getMessage(), false, null, response.getProductIds(), null));
                }
            } else if (productId != null && requiredQuantity != null) {
                isAvailable = inventoryService.isProductAvailable(productId, requiredQuantity);
                if (isAvailable) {
                    return ResponseEntity.ok(new InventoryResponse("Product is available", true, null, null, null));
                } else {
                    return ResponseEntity.status(BAD_REQUEST)
                            .body(new InventoryResponse("Product is not available in the required quantity", false, null, null, null));
                }
            } else {
                return ResponseEntity.status(BAD_REQUEST)
                        .body(new InventoryResponse("Invalid request parameters", false, null, null, null));
            }

        } catch (ResourceNotFoundException e) {
            return ResponseEntity.status(NOT_FOUND).body(new InventoryResponse(e.getMessage(), false, null, null, null));
        } catch (Exception e) {
            return ResponseEntity.status(INTERNAL_SERVER_ERROR)
                    .body(new InventoryResponse("An error occurred while checking product availability", false, null, null, null));
        }
    }
    ...
    public ResponseEntity<InventoryResponse> batchUpdateInventoryStock(
        ...
        try {
            ...
            return ResponseEntity.ok(new InventoryResponse("inventory has been updated", true, null, null, null));
        } catch (ResourceNotFoundException e) {
            return ResponseEntity.status(NOT_FOUND).body(new InventoryResponse(e.getMessage(), false, null, null, null));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(BAD_REQUEST).body(new InventoryResponse(e.getMessage(), false, null, null, null));
        } catch (Exception e) {
            return ResponseEntity.status(INTERNAL_SERVER_ERROR)
                    .body(new InventoryResponse("Error occurred while updating the inventory", false, null, null, null));
        }
    }
