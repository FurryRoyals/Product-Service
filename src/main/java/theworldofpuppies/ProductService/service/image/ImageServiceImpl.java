    ...
    public List<Image> getImagesByProductId(String productId) {
        ...
        for (String imageId : product.getImageIds()) {
            if (imageId != null && !imageId.isBlank()) {
                Image image = imageRepository.findById(imageId) // Assuming an ImageRepository exists
                        .orElseThrow(() -> new ResourceNotFoundException(imageNotFound + imageId));
                images.add(image);
            }
        }
        return images;
    ...
    public List<ImageDto> saveImages(List<MultipartFile> files, String productId, MultipartFile firstImage) {
        ...
        if (firstImage != null) {
            try {
                ...
                imageIds.add(savedFirstImage.getId());
